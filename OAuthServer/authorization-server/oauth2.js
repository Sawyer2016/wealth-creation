...
server.grant(oauth2orize.grant.code(function (client, redirectURI, user, ares, done) {
  ...
  db.authorizationCodes.save(code, client.id, redirectURI, user._id, client.scope, function (err) {
    if (err) {
      return done(err);
    }
...
server.grant(oauth2orize.grant.token(function (client, user, ares, done) {
  ...
  db.accessTokens.save(token, config.token.calculateExpirationDate(), user._id, client.id, client.scope, function (err) {
    if (err) {
      return done(err);
    }
...
server.exchange(oauth2orize.exchange.password(function (client, username, password, scope, done) {
  ...
  db.users.findByUsername(username, function (err, user) {
    ...
    var token = utils.uid(config.token.accessTokenLength);
    db.accessTokens.save(token, config.token.calculateExpirationDate(), user._id, client.id, scope, function (err) {
      if (err) {
        return done(err);
      }
      var refreshToken = null;
      ...
      if (scope && scope.indexOf("offline_access") === 0) {
        refreshToken = utils.uid(config.token.refreshTokenLength);
        db.refreshTokens.save(refreshToken, user._id, client.id, scope, function (err) {
          if (err) {
            return done(err);
          }
          return done(null, token, refreshToken, {expires_in: config.token.expiresIn});
