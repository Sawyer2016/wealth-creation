...
exports.info = [
  ...
  function (req, res) {
    ...
    if(req.authInfo.scope.indexOf('read-user-account')!== -1)
    	res.json({user_id: req.user._id, name: req.user.username, scope: req.authInfo.scope, email: req.user.email, role: req.user.role});
    else
    	res.json({error:'not authorized scope'});
  }
];
