...
exports.find = function (id, done) {
  mongodb.getUserCollection(function (collection) {
    var cursor = collection.find({_id: ObjectId(id)});
    cursor.nextObject(function (err, user) {
      if (!err && user) {
        return done(null, user, {message: 'Oops! Wrong!'});
      } else {
        return done(null);
      }
    });
 ...
 * @returns The user if found, otherwise returns null
 */
exports.findByUsername = function (_username, done) {

  mongodb.getUserCollection(function (collection) {
    var cursor = collection.find({username: _username});
    cursor.nextObject(function (err, user) {
      if (!err && user) {
        return done(null, user);
      } else {
...
exports.insertUser = function (_username, _name, _password, _email, done){
  mongodb.insertUserDocument(_username, _name, _password, _email, done);
};
